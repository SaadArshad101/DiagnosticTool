...
@Component({
  selector: 'app-dx-theme-question-display',
  templateUrl: './theme-dx-question-display.component.html',
  styleUrls: ['./theme-dx-question-display.component.css'],
})

export class ThemeDXQuestionDisplayComponent {
  toolbarOptions = [
  ...
  openDiagnosticNotesDialog(): void {
    const dialogRef = this.dialog.open(DiagnosticDXNotesModalComponent, {
      data: {
        diagnostic: this.diagnostic,
        diagnosticNotes: this.getUserData().diagnosticNotes } ,
      width: '80%'
    });
  ...
  getQuestionNumber(question: Question) {
    const firstDigit = this.diagnostic.themes.findIndex(theme => theme === this.getCurrentTheme()) + 1;
    const flatQuestions = this.getCurrentFlattenedQuestions();

    //this.flattenedQuestionMap.get(this.getCurrentThemeId()).map(obj => obj.question);
  }
